import { FEATURES, FABRIC_TAGS, FABRIC_BRANDS } from "../lib/site";
              ...
              <Reveal delay={270}>
                <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {FABRIC_BRANDS.map((brand) => (
                    <div key={brand.name} className="rounded-3xl border border-line/70 bg-ink-2 p-4">
                      <p className="text-[11px] uppercase tracking-[0.24em] text-gold">{brand.origin}</p>
                      <p className="mt-3 text-[14px] font-medium text-bone">{brand.name}</p>
                    </div>
                  ))}
                </div>
              </Reveal>
